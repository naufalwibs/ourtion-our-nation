...
const restCountryApi = "https://restcountries.com/v2/";
...
export function setCountryAsync(payload) {
  return (dispatch) => {
    ...
    fetch(`${restCountryApi}/name/${payload}`)
      .then((res) => res.json())
      .then((country) => {
    ...
    if (state.favoritesReducer.favorites.length === 0) {
      ...
      for (let i = 0; i < state.favoritesReducer.favorites.length; i++) {
        if (state.favoritesReducer.favorites[i].name === payload.name) {
          ...
          state.favoritesReducer.favorites[i].name !== payload.name &&
          i === state.favoritesReducer.favorites.length - 1
        ) {
          dispatch({ type: "favorites/addFavorite", payload });
          MySwal.fire({
            icon: "success",
